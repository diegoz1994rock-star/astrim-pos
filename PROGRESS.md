...
| M4 — Identidad y acceso | 🔄 en progreso |
...
- **Siguiente paso al retomar**: iniciar M4 (Fase 4) — módulo Login/Autenticación (`modules/auth`): caso de uso `AuthenticationService` en `application/` que usa `hash_password`/`verify_password`, cuenta intentos fallidos en `LoginAttempt` con bloqueo temporal, crea `UserSession`, y llena `SessionManager.login(...)` con los permisos efectivos del usuario (join `users` → `roles` → `role_permissions` → `permissions`). Luego la pantalla de login en `presentation/` (PySide6) y cablearla como primera pantalla real en `main.py` en vez de la ventana vacía. Seguido de inmediato por Usuarios y Roles/Permisos (ya con el esquema y `core/security` listos, son mayormente CRUD + UI).
