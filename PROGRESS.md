...
- **Siguiente paso al retomar**: iniciar M5 (Fase 4) — módulo Productos y Categorías (`modules/products`): casos de uso (crear/editar categoría jerárquica, crear producto simple/compuesto/combo con receta o ítems de combo, asignar impuestos), repositorios, UI de catálogo (tabla de productos + formulario). El esquema (`Product`, `Category`, `RecipeItem`, `Combo`, `ComboItem`, `Tax`, `ProductTax`) y los enums (`ProductType`) ya existen desde la Fase 3. Seguido de Inventario (`modules/inventory`), que consume eventos de Productos/Ventas para mantener `StockLevel` — buen momento para introducir el primer caso de uso real del bus de eventos entre módulos (ej. `ProductSoldEvent` → `InventoryModule` descuenta stock), documentando el patrón para que Ventas, Compras y Restaurante lo repliquen.
