...
| M5 — Catálogo e inventario | 🔄 en progreso (Productos y Categorías completo; Inventario pendiente) |
...
- **Siguiente paso al retomar**: módulo Inventario (`modules/inventory`) — casos de uso de entradas/salidas/transferencias/ajustes sobre `StockLevel`/`StockMovement`/`ProductBatch`/`StockAlertConfig` (el esquema ya existe desde la Fase 3). Este es el primer módulo que debe **suscribirse** a eventos de otro módulo en vez de solo publicar los propios: suscribir un handler a `ProductCreatedEvent` (ya publicado por `products`, ver `modules/products/domain/events.py`) que cree un `StockLevel` en 0 por cada bodega para productos con `track_inventory=True` — documentar dónde se registran las suscripciones (probablemente un `register_event_handlers()` por módulo, invocado desde `bootstrap_core` en `main.py`, ya que hoy ningún módulo se suscribe a nada todavía). Después: Clientes y Proveedores (`modules/customers`, `modules/suppliers`), más simples, mismo patrón CRUD ya establecido.
